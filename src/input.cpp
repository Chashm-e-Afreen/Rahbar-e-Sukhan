...
std::string remove_nuisances(std::string input) {
  std::unordered_set<std::string> urduSet{
      ...
      "ڈ", "ذ",  "ر", "ڑ", "ز",  "ژ", "س", "ش", "ص", "ض", "ط", "ظ", "ع",
      "غ", "ف",  "ق", "ک", "گ",  "ل", "م", "ن", "ں", "و", "ہ", "ھ", "ی",
      "ئ", "ئے", "ے", "ِ",  "ــِ", "ةٓ", "ۂ", "ۂ", "ۃ", "ۂ", "ؤ", "ۂ"};
...
std::vector<std::vector<std::string>> split_input(std::string input) {
  ...
  std::transform(lines.begin(), lines.end(), lines.begin(),
                 [](std::string &line) { return reduce(line); });
  std::copy_if(lines.begin(), lines.end(), lines.begin(),
               [](std::string &line) { return !line.empty(); });

  std::transform(lines.begin(), lines.end(), std::back_inserter(words),
                 [](std::string &line) { return split(line, ' '); });
  return words;
}
